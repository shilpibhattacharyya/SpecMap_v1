...
class GenerateSpectrumMap:
    ...
    def avgPairwiseMapError(self,maps_A_dBm, maps_B_dBm):
        ...
        print('error_matrix[np.ix_(map_A_assoc_indices, map_B_assoc_indices)]: ', error_matrix[np.ix_(map_A_assoc_indices, map_B_assoc_indices)])
...
if __name__ == '__main__':
    ...
    var_org = gsm.var
    sensor_count_data=[]
    ###--------------------------------EMSelectionwithvariance------------------------###

    for numsensors in range(1, target_sensors):
    
        sensor_count_data = gsm.interpolateMissingData(pca=numsensors, x_indices = gsm.training_data_x_indx,
                                                           y_indices =  gsm.training_data_y_indx,
                                                           vals = gsm.training_data_vals,
                                                           labels = gsm.training_data_labels)#would we still interpolate??I think this result should be based on the best
                                                                #algo. discovered from first part - for now it is EMI.
        iem_min_comp, iem_min_bic, iem_min_aic, iem_min_ari, iem_predicted_labels, cm, var = gsm.runGMMClustering(n_component_list = (np.array(gsm.cur_training_matrix)).shape[0]) 
    ...
    #print('ksensorsloc: ', len(gsm.training_data_y_indx))      


    var_remaining =  var_org[sensors:]
    print 'var_remaining: ' , var_remaining

    var_remaining_indices = sorted(range(len(var_remaining)), key=lambda i: var_remaining[i], reverse=True)[:target_sensors-sensors]
    print 'var remaining indices: ', var_remaining_indices
    train_matrix = []
    train_matrix_bestk = []
    train_matrix_remaining = []
    
    for mat in sensor_org_data:
        train_bestk = mat[:sensors]
        train = mat[sensors:]
        train_matrix.append(train)
        train_matrix_bestk.append(train_bestk)
	train_matrix_remaining.append(mat[sensors+1:target_sensors])

###########################################################################################################
    gsm.cur_training_matrix = np.hstack( (np.array(sensor_count_data),
                                                               np.array(train_matrix_remaining))
                                                             )

    print('final training matrix: ', (np.array(gsm.cur_training_matrix)).shape)
    ...
    gsm.displayMaps(map_list=iem_display_mapList, figFilename = './sensorselection/EMVariance_Sensel_iem.png', n_rows=2)
