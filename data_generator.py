...
class GenerateSpectrumMap:
    ...
    def generateIndividualMap(self):
        ...
        for i in range(self.tx_power_dBm.shape[0]):
            ...
            x_vals = np.arange(0, self.max_x_meter, self.d_0_meter)
            y_vals = np.arange(0, self.max_y_meter, self.d_0_meter)
            x_grid, y_grid = np.meshgrid(x_vals, y_vals, sparse=False, indexing='ij')
            dist_sq_map = (  (x_grid - tx_x)**2.0 + (y_grid - tx_y)**2.0 )
            path_loss = 10*self.n*np.log10( np.sqrt(dist_sq_map)/self.d_0_meter, where = dist_sq_map > 0.0)
    ...
    def generateTrainingData(self, n_sample, dim_ratio = 0.5, add_noise = True):
        '''
	...
	#print('indices_to_be_chosen: ', indices_to_be_chosen)
	    ...
	    #print('indx_to_locs: ', indx_to_locs)
...
if __name__ == '__main__':
    ...
    gsm.generateTrainingData(n_sample=100, dim_ratio=0.2, add_noise=False)
    #gsm.prettyPrintSamples()
    gsm.displayMaps(gsm.all_maps, './plots/gmm_all_dim.png', n_rows= 2)
    #gsm.displayTrainingDataMap(0)
