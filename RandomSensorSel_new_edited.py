...
/        '''
	...
	numSensors = int(np.round(indices_to_be_chosen*50.0/100))
	    ...
	    temp_x = np.random.choice(x_indx, numSensors)#(x_indx[0],)
	    temp_y = np.random.choice(x_indx, numSensors)#(y_indx[0],)
    ...
    def preprocessTrainingData(self, x_indices, y_indices, vals, labels):
        '''
...
def iteration4():
    ...
    gsm.preprocessTrainingData(x_indices = gsm.training_data_x_indx,
...
def iteration3():
    ...
    gsm.displayMaps(map_list=iem_display_mapList, figFilename = './rand_Sensel_iem.png', n_rows=2)
