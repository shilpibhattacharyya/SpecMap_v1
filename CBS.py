...
class GenerateSpectrumMap:
    ...
    def generateTrainingData(self, n_sample, add_noise, dim_ratio, radius):#dim_ratio):#numsensors):
        '''
        ...
        indices_to_be_chosen = max(1, int(np.round(dim_ratio*map_max_x*map_max_y)))#numsensors + 1
	numsensors = int(np.round(indices_to_be_chosen*50.0/100))
	print 'indices_to_be_chosen: ', indices_to_be_chosen
            ...
            for k in range(len(x_indx)-1):
		      ...
		      if x_indx[l] not in temp_x or  y_indx[l] not in temp_y:
                        ...
                        #print('calculated radius: ', d, x_indx[l], x_indx[k],y_indx[l], y_indx[k])
                       	if d>=radius and len(temp_x)<numsensors:##VERIFY map_max_y and map_max_x are correct choces for division.
                            #print('temp_x: ', str(d)+" >"+str(radius))
                            temp_x = temp_x+(x_indx[l],)
                            temp_y = temp_y+(y_indx[l],)
...
def iteration1():
####1st iteration-------------###
    file = open("cbs_5.txt","w")
    pca_var_ratio = 0.99999
    n_component_list = 31#np.arange(1, 10)
    ...
    map_max_x, map_max_y = gsm.all_maps[0].shape
    print map_max_x, map_max_y
    dim_ratio = 5.0/100
    k = max(1, int(np.round(dim_ratio*map_max_x*map_max_y)))
    print 'k: ', k
    A = max_y_meter*max_x_meter/k
    R = math.sqrt(A/math.pi)
    print('Disk radius:', R)
    gsm.generateTrainingData(n_sample,True, dim_ratio, R)
...
#    n_sample =  n_sample_per_config * total_distinct_config
#    
#    #gsm.generateTrainingData(n_sample, 0.3/100, False, 0,0)
#    #print('gsm.training_data_x_indx: ', len(gsm.training_data_x_indx))
#
#    #gsm.preprocessTrainingData(pca_var_ratio=pca_var_ratio, x_indices = gsm.training_data_x_indx,
#    #                                                   y_indices =  gsm.training_data_y_indx,
#    #                                                   vals = gsm.training_data_vals,
#    #                                                   labels = gsm.training_data_labels)
#    #c, min_bic, min_aic, gmm_min_ari, gmm_predicted_labels = gsm.runGMMClustering(n_component_list = n_component_list)
#    #print(c, min_bic, min_aic, gmm_min_ari, gmm_predicted_labels)
#    #gsm.prettyPrintSamples()
#    #k=2#gsm.sensors
#    #print('k: ',k)
#    map_max_x, map_max_y = gsm.all_maps[0].shape
#    print map_max_x, map_max_y
...
#
#    gsm.displayMaps(map_list=iem_display_mapList, figFilename = './CBS_Sensel_iem.png', n_rows=2)
